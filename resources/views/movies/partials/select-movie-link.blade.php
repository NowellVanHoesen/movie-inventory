<a href="/movies/{{ $movie->imdb_id }}" id="{{ $movie->imdb_id }}" class="block relative border border-gray-900 rounded-xl max-w-[185px]">
	...
	<div class="text-gray-900 p-2 absolute bottom-0 left-0 right-0 bg-white/60 rounded-b-xl">
		...
		<div class="text-sm flex flex-row items-center justify-between">
			<div>{{ date( 'Y', strtotime( $movie->release_date ) ) }} ({{ $movie->certification->name }})</div>
